...
class OllamaProvider extends LLMProvider {
    async complete(prompt) {
        try {
            const response = await axios.post(`${this.config.ollama.url}/api/generate`, { model: this.config.ollama.model, prompt, stream: false }, { timeout: 60000 });
...
export class NotasInterpreter {
    ...
    cleanNoise(commands) {
        ...
        for (const cmd of commands) {
            ...
            if (NOISE_COMMANDS.has(c) && c === lastCmd)
                continue;
            cleaned.push(cmd);
            lastCmd = c;
        }
...
`;
    ...
    async generateDraft(templateType = 'runbook') {
